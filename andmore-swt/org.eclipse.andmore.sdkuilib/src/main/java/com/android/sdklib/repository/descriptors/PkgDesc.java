...
public class PkgDesc implements IPkgDesc {
    ...
    public File getCanonicalInstallFolder(@NonNull File sdkLocation) {
        File f = LegacyFileOp.append(sdkLocation, mType.getFolderName());
        ...
        */
        ...
        case PKG_BUILD_TOOLS:
        case PKG_ADDON:
            f = LegacyFileOp.append(f, getInstallId());
            break;

        case PKG_PLATFORM:
        case PKG_SAMPLE:
        case PKG_SOURCE:
            f = LegacyFileOp.append(f, AndroidTargetHash.PLATFORM_HASH_PREFIX + sanitize(
                    getAndroidVersion().getApiString()));
            break;

        case PKG_SYS_IMAGE:
            f = LegacyFileOp.append(f,
